...
class Pagos_m extends CI_Model
{
    ...
    */
    function make_query()
    {
        $this->db->select("t1.pago_cliente_id, concat(IFNULL(t2.cliente_nombre,''),' ',IFNULL(t2.cliente_apellido,'')) cliente, DATE_FORMAT(t1.pago_cliente_dateinsert, '%d-%m-%Y %H:%i') pago_cliente_fecha, COALESCE(NULLIF(TRIM(t3.usuario_user), ''), CONCAT(IFNULL(t3.usuario_nombre,''),' ',IFNULL(t3.usuario_apellido,''))) cobrador, FORMAT(t1.pago_cliente_monto_plan, 2) pago_cliente_monto_plan, FORMAT(t1.pago_cliente_monto_total, 2) pago_cliente_monto_total, pago_cliente_estado");
        ...
        if (isset($_POST["search"]["value"])) {
            ...
            $this->db->or_like("t3.usuario_user", $_POST["search"]["value"]);
